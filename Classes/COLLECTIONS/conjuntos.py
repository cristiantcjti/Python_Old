...
assistiram = usuarios_data_science.copy() # copia rasa lista
assistiram.extend(usuarios_machine_learning) # corre uma lista e coloca em outra, apenas aadicionou
print(assistiram) # temos elementos repetidos

print(len(assistiram))

print(set(assistiram)) # set = conjunto, transforma um iterável em conjunto com elementos únicos 

print(set([1,2,3,1]))

print({4, 1,2,3,1}) # isso já é um conjunto, onde 1 não será repetido.
...
print(usuarios_machine_learning)
 ...
 # print(usuarios_machine_learning[3]) conjunto não tem indice

for usuario in set(assistiram): # conjuntos são iteráveis
  print(usuario)

usuarios_data_science = {15, 23, 43, 56}
usuarios_machine_learning = {13, 23, 56, 42}

print(usuarios_data_science | usuarios_machine_learning) # usuários que estão 'ou' em um 'ou' em outro....une os dois

print(usuarios_data_science & usuarios_machine_learning) # intercecção, retorna somente quem esta nos dois conjuntos

print(usuarios_data_science - usuarios_machine_learning) # exclui quem esta nos dois conjuntos.....  une dados unicos
...
# aula Collections 2.2
# congelar conjuntos
usuarios = {1,5,76,34,52,13,17}
len(usuarios)

usuarios.add(13) #adiciona um número, mas apenas se esse já não estiver dentro do grupo
len(usuarios)
...
usuarios = frozenset(usuarios) # congela conjunto o tornado não multável
usuarios
...
meu_texto.split() # quebra uma string, se não tem parâmetro quebra nos espaços vazios.

set(meu_texto.split()) # transforma as unidades em conjunto.

# aula Collections 2.3 Dicionário (Mapa etc)
...
aparicoes["Guilherme"] # retorna o valor

aparicoes["cachorro"]

aparicoes["xpto"] # como não tem vai dar erro

aparicoes.get("xpto", 0) # busca, se não retorna o parâmetro

aparicoes.get("cachorro", 0)

aparicoes = dict(Guilherme = 2, cachorro = 1) # podemos criar dicionários dessa forma mas não é tão usual
aparicoes

# part 2


aparicoes["Carlos"] = 1 # adiciona no dicionário

aparicoes

aparicoes["Carlos"] = 2 # altera no dicionário

aparicoes

del aparicoes["Carlos"] # deleta do dicionário

aparicoes

"cachorro" in aparicoes # verifica se chave esta no dic.

"Carlos" in aparicoes

for elemento in aparicoes: # imprimi chaves
  print(elemento)

for elemento in aparicoes.keys(): # imprimi chaves
  print(elemento)

for elemento in aparicoes.values(): # imprimi valores
  print(elemento)

1 in aparicoes.values() # verifica se valor em dic.

for elemento in aparicoes.keys(): # forma de pegar chave valor mas não é usual
  valor = aparicoes[elemento]
  print(elemento, valor)

for elemento in aparicoes.items(): # retorna padrão (chave:valor) em forma de tupla
  print(elemento)

for chave, valor in aparicoes.items(): # como retorna tupla, podemos já editar no formato chave = valor
  print(chave, "=", valor)

["palavra {}".format(chave) for chave in aparicoes.keys()] # retorna as chaves em formato de lista.


# aula Collections 2.4 Dicionário 


meu_texto = "Bem vindo meu nome é Guilherme eu gosto muito de nomes e tenho o meu cachorro e gosto muito de cachorro"
meu_texto = meu_texto.lower() # lowercase

aparicoes = {}

for palavra in meu_texto.split(): # percorre o texto e adiciona no dicionario chave : valor
  ate_agora = aparicoes.get(palavra, 0)
  aparicoes[palavra] = ate_agora + 1

aparicoes

from collections import defaultdict # define um valor default para o valor da chave

aparicoes = defaultdict(int) # int() retorna zero por padrão, dessa forma esse dic. retornará 0 por padrão

for palavra in meu_texto.split():
  ate_agora = aparicoes[palavra] # por criar com default não preciso do get mais
  aparicoes[palavra] = ate_agora + 1
...
dicionario = defaultdict(int) # exemplo de retorno padão
...
for palavra in meu_texto.split():
  #ate_agora = aparicoes[palavra]       posso abstrair agora = x += 1
  #aparicoes[palavra] = ate_agora + 1
  aparicoes[palavra] += 1
...
contas = defaultdict(Conta) # como defaultdict retorna o valor da funcao do parâmetro, posso criar um classe para retornar algo.
contas[15]
...
aparicoes = Counter()
for palavra in meu_texto.split():
  aparicoes[palavra] += 1 # conta quantas vezes a palavra e adiciona o valor e retorna um dic

aparicoes

aparicoes = Counter(meu_texto.split()) # retorna um dic com chave valor retirados de uma string

aparicoes


# # aula Collections 2.5  Testando o uso de diversas coleções
...
def analisa_frequencia_de_letras(texto):
  aparicoes = Counter(texto.lower()) # retorna um dic chave valor
  total_de_caracteres = sum(aparicoes.values()) # soma todos os caracteres
  

  proporcoes = [(letra, frequencia / total_de_caracteres) for letra, frequencia in aparicoes.items()] # calcula as proporcoes
  proporcoes = Counter(dict(proporcoes)) # retorna dic com caracteres e proporcoes
  mais_comuns = proporcoes.most_common(10) # apenas os 10 mais comuns
  for caractere, proporcao in mais_comuns: # itera para mostrar ddados
